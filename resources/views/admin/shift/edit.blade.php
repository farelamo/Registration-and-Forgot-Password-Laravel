            ...
            <form action="/admin/shift/{{ $shift->id }}" method="post">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" id="dselect-example" name="pegawai_id">
                        @foreach ($pegawai as $p)
                            @if ((old('pegawai_id') ?? $shift->pegawai_id) == $p->id)
                ...
                <div class="mb-3">
                    ...
                    <input type="date" class="form-control" id="tanggal" name="tanggal"
                        placeholder="{{ date('Y-m-d') }}" value="{{ old('tanggal') ?? $shift->tanggal }}" />
                    @error('tanggal')
                        <div class="text-danger">*{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control" id="jobdesk" name="jobdesk" placeholder="Jobdesk">{{ old('jobdesk') ?? $shift->jobdesk }}</textarea>
                    @error('jobdesk')
                        <div class="text-danger">*{{ $message }}</div>
                    @enderror
                </div>
                <div class="d-flex justify-content-end">
                    <button type="submit" class="btn btn-danger">Ubah Shift</button>
                </div>
            </form>
        </div>
    </div>
